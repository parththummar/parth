...
async function measureLatencyMs() {
  const urls = [
    `https://www.cloudflare.com/cdn-cgi/trace?ts=${Date.now()}`,
  ...
  for (const url of urls) {
    try {
      for (let i = 0; i < 3; i += 1) {
        ...
        const res = await fetch(url + (url.includes('?') ? '&' : '?') + `_=${i}`, {
          cache: 'no-store',
          mode: 'cors'
        });
  ...
  if (attempts.length === 0) {
    throw new Error(lastError?.message || 'Latency check blocked by browser or extension');
  }
...
async function measureDownloadMbps() {
  ...
  try {
    ...
    throw new Error(e?.message?.includes('blocked') ? e.message : 'Download test blocked by browser or extension');
  }
}
